...
class LLMTrainer(nnx.training.Trainer):

  def __init__(self, *, config: nnx.training.TrainConfig, workdir: str):
    super().__init__()
    self.config = config
    ...
    workdir = Path(self.workdir).absolute().resolve()
    workdir.mkdir(parents=True, exist_ok=True)
  ...
  def initialize(self) -> tuple[nnx.Module, nnx.Optimizer, nnx.Rngs]:
    ...
    if self.config.transformer_name is not None:
      model_config = transformer_lib.TransformerConfig.from_version_name(
          self.config.transformer_name,
          num_embed=self.config.vocab_size,
          dtype=dtype,
          shd_config=shd_config,
      )
    else:
      assert self.config.transformer_params is not None
      model_config = transformer_lib.TransformerConfig.from_dict(
    ...
    learning_rate_fn = create_learning_rate_schedule(
        learning_rate=self.config.learning_rate, warmup_steps=self.config.warmup_steps
    )

    rngs = nnx.Rngs(params=self.config.seed, dropout=self.config.seed)
  ...
  def train_step(
    ...
    def loss_fn(model, rngs):
      ...
      loss_per_sample = compute_weighted_cross_entropy(
          logits, targets, weights, self.label_smoothing
      )
...
def main():
  ...
  config = nnx.training.TrainConfig(
    seed=123,
    mesh_config={"fsdp": -1, "tensor": 2},
  )
  ...
  # train_and_evaluate(FLAGS.config, FLAGS.workdir, FLAGS.chpt_bucket)
